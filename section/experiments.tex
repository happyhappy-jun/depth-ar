...
and symmetrically no harm. Our evidence is also drawn from one model family. Running the deployable configuration on
Qwen3 (0.6B, 1.7B, 4B) under the same protocol, recovery is non-monotone in scale and absent at
1.7B despite that being the most damaged cell, so neither the scale trend nor the damage
ordering should be read as universal (\Cref{sec:app-qwen3}). Translating light-budget likelihood
