...
language-modelling damage that skipping causes at conservative budgets, across Qwen2.5-0.5B,
1.5B and 7B, and up to \phm{70.0}\% when layers are chosen to maximise recovery. The
recovery grows with scale, and it costs a $d$-dimensional coefficient vector per skipped
block and $O(Td)$ work. Getting it required discarding the obvious version of the idea: a
single global coefficient per layer fails almost everywhere, and at layer 3 of
Qwen2.5-0.5B substituting its prediction is \emph{worse} than dropping the update
outright. One coefficient per channel, with nothing else relaxed, is what separates a
method that works from one that does not.

\textbf{(2) Under aggressive pruning the recovered likelihood becomes recovered
behaviour.} At a matched skipping fraction, on layer sets where plain skipping has driven
language-modelling loss to \phm{5.99} against a dense \phm{2.33} and collapsed downstream
accuracy, \mname answers \phm{132} more questions correctly out of \phm{900} than plain
skipping does, and \phm{90} more at the larger budget. These are the only \phm{2} of
\phm{16} accuracy comparisons we run that survive Bonferroni correction, and they are the
regime in which the choice of skip operator matters most: when a model must be compressed
hard, predicting the missing update rather than deleting it is worth a large and
measurable number of answers.

\textbf{(3) Predictability is not recoverability.} Held-out explained update energy
$P_\ell$, the cheap signal this method appears to hand you for free, is uncorrelated with
the damage a predictor actually repairs (Spearman \phm{$-0.015$}, $p=\phm{0.95}$,
$n=\phm{22}$). Layers must be selected against measured recovery, not against
predictability, and the same blindness recurs when an energy-weighted criterion is used to
tune the predictor itself.

At conservative budgets, where plain skipping already preserves most of the model, the
likelihood \mname recovers does not yet register as a downstream accuracy gain: the
differences there are within sampling noise in either direction at all three scales. We
report that plainly in \Cref{sec:limitations}, and regard turning conservative-budget
likelihood recovery into conservative-budget accuracy as the natural next step.
