...
predictor exploits that.} \mname recovers 4.6\% to 23.3\% of the
language-modelling damage that skipping causes at light budgets across Qwen2.5-0.5B, 1.5B and
7B, and up to 70.0\% when layers are chosen to maximise recovery, at a $d$-dimensional
...
\textbf{(2) Across Qwen2.5, the downstream benefit scales with the damage skipping inflicts.} At light
...
Bonferroni correction across all 63 comparisons we run. This holds under the deployable,
damage-based selection rule, not merely under an oracle: at 28.6\% of blocks skipped on
Qwen2.5-7B, \mname answers 89 more questions correctly out of 900 than plain
skipping ($z=4.31$) and recovers 38.1\% of its language-modelling damage. It also
holds under damage-seeking selection on Qwen2.5-1.5B (132 and 90 answers). The
rule is simple: prediction pays off in proportion to what skipping destroys. A first
out-of-family test on Qwen3 bounds that ordering rather than confirming it
(\Cref{sec:app-qwen3}).
...
predictor actually repairs (Spearman $-0.015$, $p=0.95$, $n=22$), so layers
