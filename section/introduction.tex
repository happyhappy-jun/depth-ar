...
recovery grows with scale at four skipped blocks, and it costs a $d$-dimensional
coefficient vector per skipped block and $O(Td)$ work. Getting it required discarding the obvious version of the idea: a
...
$n=\phm{22}$), so layers must be selected against measured recovery. At conservative
budgets the recovered likelihood does not yet register as a downstream accuracy gain; we
report that plainly in \Cref{sec:limitations} and regard closing it as the natural next
step.
