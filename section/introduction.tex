...
Qwen2.5-1.5B, and more when layers are selected to maximise it. Downstream accuracy
follows it only where the model is already ruined. Choosing layers to maximise
recovery does produce a large, unambiguous accuracy gain --- \phm{132} additional
correct answers out of \phm{900} on 1.5B --- but it does so on a layer set at which
plain skipping has driven language-modelling loss to \phm{5.99} against a dense
\phm{2.33}. At the operating point one would deploy, \mname's effect on accuracy is
\emph{undetectable}: across both scales and all budgets, every delta lies within
one binomial standard error of zero, ranging from \phm{$-6$} to $+\phm{5}$ answers,
with several tasks exactly tied. We state these as counts, not as recovered-gap
fractions, because the fractions divide by Dense-minus-Plain-Skip denominators as
small as \phm{27} answers: five answers become ``$18.5\%$ of the accuracy gap
recovered'', a figure that would sit comfortably in an abstract and means nothing.
We therefore claim no downstream improvement at the deployable operating point ---
and, symmetrically, no downstream harm. At these sample sizes there is nothing to
claim in either direction, and that is itself the result.
