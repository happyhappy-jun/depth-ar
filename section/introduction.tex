...
damage that skipping inflicts, so prediction pays off in proportion to what skipping destroys,
...
light budgets across Qwen2.5-0.5B, 1.5B and 7B, and up to 70.0\% when layers are chosen to
maximise recovery, for 0.64\% added prefill latency. Where skipping is destructive, that recovery
reaches behaviour: at 28.6\% of blocks skipped on Qwen2.5-7B, under the deployable damage-based
rule, \mname answers 89 more questions correctly out of 900 than plain skipping ($z=4.31$), one
of only 4 of 63 comparisons to survive Bonferroni correction. Held-out predictability, the cheap
signal this method appears to hand you for free, is uncorrelated with the damage a predictor
actually repairs (Spearman $-0.015$, $p=0.95$), so layers must be selected against measured
recovery.
