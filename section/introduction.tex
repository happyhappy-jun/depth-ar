...
every fitted coefficient is negative across layers 4--16, and \ph{88}\% of the
...
actually repairs (Spearman \ph{$-0.015$}, $p=\ph{0.95}$, $n=\ph{22}$). Selecting
...
\textbf{Depth is anti-correlated before it is autoregressive}, as above --- a fact
about Transformer depth that is independent of our method and that any future
update-prediction scheme has to contend with. Selecting by measured recovery,
\mname repairs up to \ph{24.7}\% of the language-modelling damage plain skipping
causes at matched depth, against \ph{16.3}\% for the scalar predictor.
