...
\phm{16} WikiText-103 sequences of length \phm{512}; $P_\ell$ and the NLL-recovery
fractions are scored on \phm{16} \emph{disjoint held-out WikiText-103} sequences. This is
...
\phm{16} WikiText-103 sequences, but NLL is reported on the \emph{WikiText-2 test} split
(\phm{32} sequences), and downstream accuracy on \phm{100} examples per task.

\paragraph{1.5B and \topscale.} Coefficients are fitted on \phm{32} WikiText-103
sequences of length \phm{1024}; NLL is reported on the WikiText-2 test split and accuracy
on \phm{300} examples per task.
...
resolvable; at 2048 the \emph{prefill} overhead is \phm{1.0}\,ms against interquartile ranges of
...
difference of two proportions, and report absolute counts in the body. Of the \phm{63}
comparisons, \phm{59} are not significant at the two-sided $95\%$ level. A $1$-SE band is
not a significance test: a delta at $1.0$--$1.2$ SE has $p \approx 0.25$ and establishes
nothing.

Only \phm{4} comparisons are significant, and both survive Bonferroni correction over the
family of \phm{63} tests actually performed ($\alpha/m = \phm{0.00079}$, $|z| \ge
\phm{3.35}$): recovery-maximising selection on Qwen2.5-1.5B at $k=2$
...
than being ceremonial: an uncorrected $95\%$ threshold over \phm{63} tests would expect
about \phm{3.15} false positives by chance, so the uncorrected borderline case
...
\phm{4.44e-16}$, one ulp), but multiple-choice items whose top two options are nearly tied can
...
seed 42, same layers, evaluated once at \phm{32} rows per batch and once at \phm{8}) moves up
to \phm{4} answers of \phm{900}, with nothing else changed. That is the same magnitude as
...
Four of \phm{63} comparisons survive correction: Qwen2.5-7B under damage-based selection at
$k=8$ (\phm{89} answers, $z=\phm{4.31}$; and \phm{74} for the cross-token extension,
$z=\phm{3.57}$), and Qwen2.5-1.5B under damage-seeking selection at $k=2$ and $k=4$
(\phm{132} and \phm{90} answers, $z=\phm{6.45}$ and $\phm{4.37}$). The 1.5B survivors are
\phm{33}$\times$ and \phm{22.5}$\times$ the measured jitter. No reordering of the evaluation
batch produces effects of that size.

\paragraph{The axis is damage, not selection rule.} Across all \phm{28} skipping runs in this
paper, the recovery \mname achieves correlates with the damage plain skipping does: Spearman
$\rho = \phm{0.820}$ ($p = \phm{9.0e-08}$) between plain-skip NLL damage and \mname's NLL
gain, and $\rho = \phm{0.468}$ ($p = \phm{0.012}$) on the accuracy axis. \emph{We report this
as descriptive ordering, not as a fitted law.} The \phm{28} runs are not independent: budgets
...
recovery earns \mname's largest likelihood recovery anywhere (\phm{70.0}\% at $k=2$ on
1.5B, at an absolute loss of \phm{3.43} against a dense \phm{2.33}) and its only
significant accuracy gains, on models whose absolute quality is unacceptable. The rule
that keeps quality acceptable earns a real likelihood recovery (\phm{23.3}\% at $k=4$ on
...
At layer 3 the fitted scalar is \phm{$+1.10$} while the median channel coefficient is
\phm{$-0.20$} and \phm{76}\% of channels are negative, so the scalar moves three quarters of
...
\label{eq:crosstoken}
\end{equation}
costs \phm{1792} scalars per skipped block on Qwen2.5-0.5B. This is \emph{twice the
...
\cref{eq:crosstoken} to \cref{eq:diag} exactly, which we verify numerically (\phm{0.0}
absolute NLL difference).

On a single-layer scan over the \phm{22} eligible layers of Qwen2.5-0.5B, cross-token
prediction lowers mean held-out NLL from \phm{3.40} to \phm{3.08} against a dense
\phm{2.76}, and beats the depth-only predictor at \phm{19} of the \phm{22} layers, raising
median single-layer recovery by \phm{1.8}$\times$.
...
orders of magnitude of ridge strength its recovery never rises above \phm{$-2.42$}
(\Cref{sec:app-ridge}), and it sits at \phm{$-3.31$} at the fitted value. Admitting the
previous token's update turns that same layer from \phm{$-3.31$} to \phm{$+0.19$}. The
...
improve on \cref{eq:diag} in any way that matters. On Qwen2.5-0.5B it recovers \phm{4.8}\%
of the plain-skip NLL damage at $k=4$ against \phm{4.6}\% for the depth-only predictor, and
\phm{18.3}\% against \phm{18.0}\% at $k=6$; on Qwen2.5-1.5B, \phm{8.9}\% against \phm{8.2}\%
at $k=2$ and \phm{10.8}\% against \phm{10.3}\% at $k=4$. Downstream accuracy is at noise
...
has already discarded: its three largest gains over \cref{eq:diag} are at layers \phm{2},
\phm{21} and \phm{1}, while the deployable sets are $\{8,10,13,18\}$ and
...
\emph{robustness}: its worst layer anywhere in the scan recovers \phm{$-0.027$}, against
\phm{$-3.314$} for the depth-only predictor. It removes the failure mode rather than
...
holds: these are the most damaged sets in the paper (plain-skip NLL \phm{6.59} at $k=2$ and
\phm{9.58} at $k=4$, against a dense \phm{2.76}), and the cross-token extension recovers
\phm{66.8}\% and \phm{63.7}\% of that damage, its largest recoveries anywhere and roughly ten
points above the depth-only predictor's \phm{56.3}\% and \phm{54.7}\%. Accuracy moves in the predicted direction as well, by
\phm{16} to \phm{23} correct answers of \phm{300}, but at this sample size it is
...
It does not widen forever. On Qwen2.5-7B at \phm{42.9}\% of blocks skipped, the depth-only
predictor \emph{breaks down}: it recovers \phm{$-6.0$}\% of the plain-skip damage, which is to
say it is worse than simply dropping the updates (NLL \phm{6.43} against plain skipping's
\phm{6.18}). We report this rather than truncating the sweep at the last budget that flatters
the method. At that same point the cross-token extension remains above plain skipping
(\phm{7.1}\% recovery, NLL \phm{5.88}), and this is the closing half of the selection story
told earlier. At mild budgets, damage-aware selection avoids the early layers on which
cross-token information helps, so the extension has nothing to add. At \phm{42.9}\% skipped
...
\midrule
0.5B & 6 & \phm{25.0}\% & \phm{4.45} & \phm{4.15} & \phm{18.0}\% & {--} \\
0.5B & 8 & \phm{33.3}\% & \phm{5.93} & \phm{4.94} & \phm{31.3}\% & {--} \\
0.5B & 10 & \phm{41.7}\% & \phm{10.29} & \phm{7.09} & \phm{42.6}\% & {--} \\
1.5B & 6 & \phm{21.4}\% & \phm{3.24} & \phm{3.16} & \phm{8.6}\% & {--} \\
1.5B & 8 & \phm{28.6}\% & \phm{4.08} & \phm{3.89} & \phm{11.1}\% & \phm{11.6}\% \\
1.5B & 10 & \phm{35.7}\% & \phm{5.43} & \phm{4.83} & \phm{19.6}\% & \phm{19.8}\% \\
1.5B & 12 & \phm{42.9}\% & \phm{6.39} & \phm{5.69} & \phm{17.3}\% & \phm{17.4}\% \\
7B & 8 & \phm{28.6}\% & \phm{3.71} & \phm{3.05} & \phm{38.1}\% & \phm{38.1}\% \\
7B & 12 & \phm{42.9}\% & \phm{6.18} & \phm{6.43} & \phm{-6.0}\% & \phm{7.1}\% \\
\bottomrule
\end{tabular}
\end{table}
