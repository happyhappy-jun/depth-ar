...
(132 and 90 answers, $z=6.45$ and $4.37$). The 1.5B survivors are
33$\times$ and 22.5$\times$ the measured jitter. No reordering of the evaluation
...
configuration ($k=4$, damage-based selection) on Qwen3 at 0.6B, 1.7B and 4B, under the
...
\textbf{The scale trend does not replicate.} Recovery is non-monotone across the Qwen3 ladder:
$+20.5\%$ at 0.6B, $-1.9\%$ at 1.7B and $+4.7\%$ at 4B. The growth with scale we report on
Qwen2.5 at $k=4$ is therefore a property of that family, not a general property of Transformer
depth, and we scope the claim accordingly rather than confining our evidence to the family that
shows it.
...
specific rather than total. We have no explanation for the 1.7B cell and we do not offer one.
It is an out-of-sample counterexample to the ordering that organizes this paper, it was found
by the first test we ran outside the family the ordering was built on, and it is reported here
with that weight. The correlation of \Cref{sec:app-noise} should be read as what we said it
was: a description of the runs we performed, not a law.
