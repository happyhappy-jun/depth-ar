...
difference of two proportions, and report absolute counts in the body. Of the \phm{41}
comparisons, \phm{37} are not significant at the two-sided $95\%$ level. A $1$-SE band is
...
family of \phm{41} tests actually performed ($\alpha/m = \phm{0.00122}$, $|z| \ge
\phm{3.23}$): recovery-maximising selection on Qwen2.5-1.5B at $k=2$
...
than being ceremonial: an uncorrected $95\%$ threshold over \phm{37} tests would expect
about \phm{2.05} false positives by chance, so the uncorrected borderline case
...
Four of \phm{41} comparisons survive correction: Qwen2.5-7B under damage-based selection at
...
\paragraph{The axis is damage, not selection rule.} Across all \phm{26} skipping runs in this
paper, the recovery \mname achieves correlates with the damage plain skipping does: Spearman
$\rho = \phm{0.813}$ ($p = \phm{4.4e-07}$) between plain-skip NLL damage and \mname's NLL
gain, and $\rho = \phm{0.486}$ ($p = \phm{0.012}$) on the accuracy axis. \emph{We report this
as descriptive ordering, not as a fitted law.} The \phm{26} runs are not independent: budgets
