...
Coefficients are fitted on \ph{32} unlabeled C4 sequences of length \ph{512}
\citep{raffel2020exploring}, and $P_\ell$ is scored on a disjoint held-out split
of the same size. Statistics are accumulated in FP64 (\cref{eq:fit}) so that no
hidden states need to be stored. Task evaluation uses fixed example IDs with seed
42 throughout. Discovery and verification runs (0.5B, 1.5B) are executed in FP32
