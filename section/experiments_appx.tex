...
difference of two proportions, and report absolute counts in the body. Of the \phm{37}
comparisons, \phm{35} are not significant at the two-sided $95\%$ level. A $1$-SE band is
...
family of \phm{37} tests actually performed ($\alpha/m = \phm{0.00135}$, $|z| \ge
\phm{3.20}$): recovery-maximising selection on Qwen2.5-1.5B at $k=2$
...
about \phm{1.85} false positives by chance, so the uncorrected borderline case
...
The two Bonferroni survivors are unaffected, and we state the ratio rather than assert
immunity: at \phm{132} and \phm{90} net answers they are \phm{33}$\times$ and
\phm{22.5}$\times$ the measured jitter. No reordering of the evaluation batch produces
effects of that size.
...
cells, at damage-based selection, are below. \mname's advantage over plain skipping widens as
compression deepens: the recovered fraction of plain-skip NLL damage rises monotonically with
the skipping fraction on Qwen2.5-0.5B, and broadly so on 1.5B. Recovery-maximising selection
is excluded from this table, since the models it produces are not ones anyone would deploy
(\Cref{sec:app-noise}).
...
is the fraction of the plain-skip damage \mname repairs.}
\label{tab:pareto}
\begin{tabular}{llccccc}
\toprule
Model & $k$ & blocks skipped & Plain Skip NLL $\downarrow$ & \mname NLL $\downarrow$ & recovered $\uparrow$ \\
\midrule
...
1.5B & 8 & \phm{28.6}\% & \phm{4.08} & \phm{3.89} & \phm{11.1}\% \\
1.5B & 10 & \phm{35.7}\% & \phm{5.43} & \phm{4.83} & \phm{19.6}\% \\
1.5B & 12 & \phm{42.9}\% & \phm{6.39} & \phm{5.69} & \phm{17.3}\% \\
\bottomrule
\end{tabular}
\end{table}
