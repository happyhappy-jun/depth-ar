...
free estimator is available --- reuse the previous update, $\widehat{\dd}_\ell =
\dd_{\ell-1}$ --- and we carry this \textbf{Copy Update} baseline throughout,
...
single forward pass over 16--\phm{32} sequences. Padding tokens are excluded from
the accumulators.

\paragraph{Which layers to skip.} The natural companion to \cref{eq:fit} is the
held-out fraction of update energy a predictor explains,
\begin{equation}
P_\ell = 1 -
...
\label{eq:pscore}
\end{equation}
which is computable from the dense model on unlabeled text before any skipping
decision is taken, and which is an obvious candidate for ranking layers. We
evaluate it as such, alongside two selection rules that instead measure the effect
of skipping directly on a small development set: ranking by lowest single-layer
NLL damage, and ranking by the damage a predictor actually repairs. \Cref{sec:experiments}
shows the choice between them matters more than the choice of predictor --- and
that the cheap one is the wrong one.

\paragraph{Cost.} \mname stores a $d$-dimensional coefficient vector per skipped
block ($d = \phm{896}$ on Qwen2.5-0.5B), against the $d^2$ parameters a dense
boundary map between hidden states would require --- three orders of magnitude
fewer. Inference cost is unchanged in order: one elementwise multiply and one add,
